  ...
  useEffect(() => {
    ...
    if (filters.series.length > 0) {
      filters.series.forEach((filter) => {
        ...
        const foundObj = products.find((item) => item.sectionName == filter);
        ...
        <div className="lg:grid grid-cols-[20%80%] lg:space-x-10 container">
          {products && (
            ...
            >
              ...
              {getKolichestvo(products).map((obj, index) => {
                return (
                  <Checkbox
                    key={`${obj.name}`}
                    text={`${obj.name} ${products[0].itemUnit} (${obj.length})`}
                    id={`liters-${obj.name}`}
                    // quantity={2}
                    filters={filters}
...
export async function getServerSideProps(context) {
  ...
  return {
    props: {
      products: JSON.parse(JSON.stringify(products.filteredData)),
      sectionRoute: products.sectionRoute,
    },
  };
}
